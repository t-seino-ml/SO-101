...
![リーダ機で動かす画面](docs/images/ui-teleop.png)

もう一方のアームを手で動かすと、ロボットが同じ形について来ます。モデルも検出も
軌道も使いません。外付けカメラとアーム搭載カメラの映像が出ます。

### ブロックをつかむ

![ブロックをつかむ画面](docs/images/ui-pick.png)

色を選ぶと、側面カメラがその色を探し、いちばん近い Slot の教示済み軌道を再生して
ブロックを缶へ入れます。3 枚目のパネルに検出結果（バウンディングボックスと
Slot の円）が重なり、いま何をしているかは右上のバッジと下段の見出しに出ます。
...
**会場で合わせ直すのは Slot の位置だけです。所要 5 分、軌道の教示はやり直しません。**

なぜそれだけで済むかというと、判定がすべてピクセル空間で完結しているからです。
カメラがどこから見ているかは Slot の位置として吸収され、アームは教示済みの
関節角をそのまま再生します。カメラとアームの位置関係を測る工程がありません。

### 1. 配置

| 手順 | 確認すること |
|---|---|
| マットを敷く | **ロボット・Slot・缶の相対位置**が教示時と同じであること。これだけが条件です |
| ロボットを ROBOT 印へ | ベースが机に**密着**していること。浮くと全体がずれます |
| 缶を CAN 印へ | |
| カメラを設置 | **位置は教示時と違って構いません。** 6 つの Slot とその上のブロックが画面に入っていれば十分です |

カメラは近すぎないほうが安定します。Slot どうしが画像上で離れて見えるほど判定が楽になります。

```powershell
uv run scripts/find_cameras.py --no-preview     # 側面カメラが見えるか
```

### 2. Slot を登録する（校正はこれだけ）

```powershell
uv run scripts/calibrate_demo_slots.py --slots 6 --detect
```

カメラ画像が開くので、**Slot1 から Slot6 まで順に中心をクリック**し、ENTER で保存します。

- **クリックした順番が Slot 番号になります。** `data/demo_slots/slot1.json` を教示した
  場所を最初にクリックしてください
- `BACKSPACE` で 1 つ取り消し、`ESC` で中止
- 各 Slot に描かれる円が「この中なら同じ Slot」の範囲です
- **円が重なる場合は半径を自動で詰めます。** 手で計算する必要はありません
...
`→ slot2（中心から 23 px）` のように出れば通っています。
...
通ったら `uv run app/demo_ui.py` を起動して展示開始です。

> **教示したブロックの位置を物理的に動かした場合は、その Slot の軌道を教示し直します**
> （`uv run scripts/teach_demo_slot.py --slot 2`）。Slot の登録だけでは追従できません。

各手順の詳細、判定を拒否したときの読み方、動かないときの対処は
[docs/08-demo.md](docs/08-demo.md) にあります。
