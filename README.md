...
SO-101（LeRobot / Feetech STS3215）のセットアップ、動作確認、テレオペレーション用のスクリプト集です。

Windows + USB シリアル環境で LeRobot 公式コマンドがそのままでは動かない箇所を回避し、キャリブレーションからテレオペまで通せる状態にしてあります。
...
`.venv` が作られ、これ 1 つで全スクリプトが動きます。pyserial は LeRobot の依存に含まれるため、追加の環境は不要です。

WSL から実行する場合は、パス区切りを `/` にすれば Windows 実行ファイルをそのまま起動できます（COM ポートは Windows プロセス側で開くため問題ありません）。

```bash
./.venv/Scripts/python.exe ports.py
```

PowerShell の場合は `\` 区切りです。
...
```bash
./.venv/Scripts/python.exe ports.py         # 認識されているシリアルポート一覧
./.venv/Scripts/python.exe scan_servos.py   # 各ポートのサーボを ping
```

両アームで ID 1〜6 が応答すれば正常です。ボーレートは 1,000,000 / 500,000 / 115,200 の順に自動判定します。

```bash
./.venv/Scripts/python.exe diag.py          # 角度・可動域・トルク・負荷・電圧・温度
```
...
```bash
./.venv/Scripts/python.exe check_stored_cal.py save
```
...
```bash
./.venv/Scripts/python.exe easy_calibrate.py --robot.type=so101_follower --robot.port=COM4 --robot.id=follower
./.venv/Scripts/python.exe easy_calibrate.py --teleop.type=so101_leader  --teleop.port=COM3 --teleop.id=leader
```
...
```bash
./teleop.sh --robot.max_relative_target=5   # 1 ステップ 5 度に制限
```

ポートや設定は環境変数で上書きできます。

```bash
FOLLOWER_PORT=COM5 LEADER_PORT=COM6 FPS=60 ./teleop.sh
```
...
| ファイル | 用途 |
|---|---|
| `ports.py` | シリアルポートの列挙と自動検出。CH34x を優先 |
| `scan_servos.py` | 各バスのサーボを ping し、ボーレートを自動判定 |
| `diag.py` | 全サーボの角度・可動域・トルク・負荷・電圧・温度を表示 |
| `monitor.py` | 手動操作を検出し、リーダー機とフォロワー機を判別 |
| `check_stored_cal.py` | サーボ内蔵キャリブレーションの表示・保存・差分確認 |
| `move_test.py` | 1 関節ずつ往復させる駆動テスト。負荷・温度で自動中断 |
| `bench_latency.py` | バスの sync_read レイテンシを実測 |
| `easy_calibrate.py` | 中間姿勢の指定が不要なキャリブレーション |
| `calibrate.py` | LeRobot 公式キャリブレーション（通信リトライのみ追加） |
| `teleoperate.py` | テレオペ本体。通信リトライと P ゲイン調整を追加 |
| `teleop.sh` | 検証済み設定でテレオペを起動するランチャー |
| `bus_patch.py` | LeRobot のシリアル通信パッチ（上記から読み込まれる） |
| `sts3215.py` | Feetech STS3215 の最小プロトコル実装 |

`move_test.py` は安全のためポートの明示指定が必須です。1 関節ずつ 1.5 度刻みで動かし、負荷が 800（フルスケール 1023）または温度が 60℃ を超えると中断します。トルクは `finally` で必ず解除されるため、中断してもアームは脱力状態で残ります。

```bash
./.venv/Scripts/python.exe move_test.py COM4 --amplitude-deg 25
```

## LeRobot 公式コマンドをそのまま使わない理由

`bus_patch.py` で 2 つの問題を回避しています。
...
```bash
SO101_P_COEFFICIENT=32 ./teleop.sh   # サーボ既定値。振動したら下げる
```
